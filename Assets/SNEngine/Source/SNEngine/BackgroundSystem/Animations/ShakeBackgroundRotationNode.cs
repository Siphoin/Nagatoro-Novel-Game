...
{
    public class ShakeBackgroundRotationNode : AsyncNode
    {
        [Input(ShowBackingValue.Unconnected), SerializeField] private float _duration = 0.5f;
        [Input(ShowBackingValue.Unconnected), SerializeField] private float _strength = 90;
        [Input(ShowBackingValue.Unconnected), SerializeField] private int _vibrato = 10;
        [Input(ShowBackingValue.Unconnected), SerializeField] private bool _fadeOut = true;
        ...
        public override async void Execute()
        {
            ...
            bool inputFadeOut = GetInputValue(nameof(_fadeOut), _fadeOut);

            var service = NovelGame.Instance.GetService<BackgroundService>();
            await service.ShakeRotation(inputDuration, inputStrength, inputVibrato, inputFadeOut);
            StopTask();
        }
    }
}
...
{
    public class ShowCharacterNodeWorker : SNILWorker
    {
        public override void ApplyParameters(BaseNode node, Dictionary<string, string> parameters)
        {
            // Проверяем, что это действительно ShowCharacterNode или его наследник
            if (!(node is ShowCharacterNode showNode))
            {
                return;
            }
